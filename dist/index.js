...
// HELPERS
// ==============================
function findLoreRoot(startDir) {
        ...
        tools: [
            {
                ...
                description: "Record an architectural decision. Call this whenever you make a technical choice that affects the codebase structure.",
                inputSchema: {
                    type: "object",
                    properties: {
                        category: {
                            type: "string",
                            description: "Category: database, authentication, architecture, api, testing, deployment, security, performance, other",
                        },
                        decision: {
                            type: "string",
            ...
            {
                name: "get_context",
                description: "Get all architectural decisions for this project. Call this at the START of every session to understand the codebase.",
                inputSchema: {
                    type: "object",
                    properties: {
                        category: {
            ...
            {
                name: "get_gaps",
                description: "Find architectural gaps — decisions that were made but may not be fully implemented.",
                inputSchema: {
                    type: "object",
                    properties: {},
                },
...
server.setRequestHandler(types_js_1.CallToolRequestSchema, async (request) => {
    ...
    if (name === "record_decision") {
        ...
        const exists = store.decisions.some((d) => d.decision.toLowerCase().trim() ===
    ...
    if (name === "get_context") {
        ...
        // filter by category
        if (args?.category) {
            decisions = decisions.filter((d) => d.category
                .toLowerCase()
        ...
        if (args?.query) {
            ...
            decisions = decisions.filter((d) => d.decision.toLowerCase().includes(q) ||
                d.reason.toLowerCase().includes(q) ||
                d.category.toLowerCase().includes(q));
        }
        const summary = decisions
            .map((d) => `[${d.category.toUpperCase()}] ${d.decision}\n` +
            `  → Reason: ${d.reason}\n` +
            `  → Constraints: ${d.constraints.length > 0
                ? d.constraints.join(", ")
                : "none"}`)
            .join("\n\n");
        return {
            content: [
                {
                    type: "text",
                    text: `LORE Context — ${decisions.length} decisions\n` +
                        `${"─".repeat(40)}\n\n` +
                        summary,
                },
            ],
    ...
    if (name === "get_gaps") {
        const store = readStore();
        const gaps = store.decisions.filter((d) => d.constraints.length > 0);
        if (gaps.length === 0) {
            return {
                content: [
                    {
