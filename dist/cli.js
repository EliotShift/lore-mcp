...
function extractDecisions(projectPath) {
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            if (deps["hono"]) {
                decisions.push(makeDecision("architecture", "Hono as web framework", "Found hono in dependencies", "HIGH"));
            }
            // Testing
...
async function decide(reason) {
    ...
    const decision = makeDecision("architecture", reason, "Manually recorded by developer", "HIGH", [], []);
    // Mark as human-written
    decision.author = "human";
    decision.human_why = true;
